...
3. Upload that PNG to Magnific with the upload bridge in `magnific.md`: `creations_request_upload` for a presigned PUT URL, `curl -X PUT --upload-file frame-final.png "<presigned-url>"` for the raw bytes, then `creations_finalize_upload` to register it. The returned creation's `identifier` is what you pass as `keyframes.start.url` on the next segment's `video_generate` call. Never pass `webUrl` there; it is not the same thing.
...
Before ANY generation, check the exact price of the exact call you plan with `simulate_cost`. It is free and read-only. Tell the user each price in plain Portuguese before it moves: the starting frame's price before the frame, then the video model menu with real prices once the frame is approved, before any video credits move. The cheap step first, the big decision second, every number real.

**Never write a credit number into a plan from memory.** Magnific's prices differ by model and change over time, so a number you did not preflight in this session is a guess dressed as a fact. Preflight it or do not say it.

**The video model is a real choice, and the user makes it.** `video_models_list` gives the current lineup, and the price spread across models on identical parameters (same duration, same resolution) is wide. So preflight the SAME planned shot across the top two or three with `simulate_cost`, then present the real numbers with the honest tradeoff: the pricier model buys the highest ceiling; a mid-priced one is legitimate and turns a small balance from one shot into several, which changes the video gate from frightening into a normal creative decision. Their money, their choice, made before anything is spent.

For a chained journey, preflight the whole chain and present the full total up front, per segment.

## A note on prompt language

Every prompt in this file is written and sent in English, even though the site and every message to the user are in Brazilian Portuguese. The generation models were trained on English and Portuguese prompts land measurably worse. The user never reads these prompts, so nothing is lost. This is the single carve-out in the language contract at the top of `SKILL.md`.

One thing that does cross over: when the scene is visibly Brazilian, say so in the English prompt. "A small bakery on a Brazilian street, Sao Paulo, warm afternoon light" lands a very different and much more correct world than "a small bakery". Without it the model defaults to American and European references, and the target audience notices immediately.
