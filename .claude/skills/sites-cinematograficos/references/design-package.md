...
## 10. The language line

Every line of copy in this package is written in Brazilian Portuguese, final and ready to ship: the band map captions, the static-hero block, every section below the fold, the FAQ, the form microcopy, and the footer. The package is where the Portuguese is written and judged; the build only wires it in. Two things travel with it:

- **Length.** Portuguese runs roughly 20 percent longer than the equivalent English line. Write to the space the band actually has, and check the longest line against the narrowest phone width before calling the package done.
- **Accents.** Name the type trio's exact faces here and confirm each one carries full Latin-1 accents. A display face that breaks on "ã" or "ç" is not a candidate, however good it looks in a specimen.

The generation prompts written in section 4 are the one exception: those stay in English, per the language contract in `SKILL.md`.
