...
| Symptom | Cause | Fix |
|---|---|---|
| Magnific tools missing right after connecting | A freshly added connector sometimes loads only on the next app start | Close Claude Code completely, reopen, return to the same chat, and verify again with `account_balance` |
...
| The Magnific connector shows it needs authentication | The sign-in did not complete during the add | Finish it in the connectors panel: the browser opens to Magnific to authorize once, then re-verify with `account_balance` |
| Magnific sign-in seems to go nowhere | The authorization opened in a different browser than the user normally uses | Warn them in advance and have them check their other browsers |
| `video_generate` is rejected or produces the wrong thing | `keyframes.start.url` was given a `webUrl` instead of a creation `identifier`, or `duration` was omitted with an explicit `slug` | Pass the `identifier`, and always send `duration` when you send `slug`. See `magnific.md` |
| `video_generate` refuses a clip carrying both a start frame and references | Start frames and `references[]` are mutually exclusive | Drop the references; this pipeline always uses the start frame |
| Magnific tools present but calls fail | Connector added but not authorized, or no credits | Re-run the sign-in flow; verify with `account_balance`, which should return a real plan and a real number |
...
| A generated scene looks foreign: US outlets, foreign plates, foreign signage | The model defaulted to English-language training references | Name the country and the city in the English prompt on the re-roll. See the note at the end of `prompt-laws.md` |
...
## Publishing on HostGator

| Symptom | Cause | Fix |
|---|---|---|
| The address shows a directory listing instead of the site | The project FOLDER was zipped instead of its contents, so everything sits one level down after extraction | In the file manager, open the folder that appeared, select everything inside, and use Mover to bring it up one level. Then re-zip correctly next time: `index.html` at the top of the zip |
| The address shows nothing, or the old site | The zip was uploaded but never extracted, or it went into the wrong folder | Confirm the destination folder against cPanel → Domínios (main domain is `public_html`, an addon has its own folder), then right-click the zip and Extrair |
| The cPanel upload fails or stalls near the end | The zip is at or over the shared-plan upload ceiling | Go back to Phase 7 and bring the hero video down; 720p on a scrubbed video is nearly invisible. FTP is the last resort, not the first |
| Every accent on the live page renders as a box or as Ã§ | The file was patched with a shell one-liner that read it in the wrong encoding, or the page is missing `<meta charset="utf-8">` | Restore the file, re-patch with the editor tool only, confirm the charset meta is present, and re-verify with `curl -s <url> \| head -c 400` |
| The browser shows a security warning right after the first publish | The free certificate is still being issued on a domain that has never served a site | Wait; it usually resolves within minutes and sometimes takes hours. If it persists, cPanel → SSL/TLS Status → Executar AutoSSL |
| The site is live but the user still sees the old version after a change | Browser cache | Hard-refresh (Ctrl+Shift+R, or Cmd+Shift+R on Mac), and verify from your side with `curl` against a string you know changed |
| The domain does not answer at all | It was registered minutes ago and DNS is still propagating | Nothing to fix. Say so plainly and check again later; this is not a broken build |
