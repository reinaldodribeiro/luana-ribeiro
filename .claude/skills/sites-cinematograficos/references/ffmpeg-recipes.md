...
The frame that becomes the next segment's start frame must be a full-quality PNG. The `-q:v 2` review jpgs above are for your eyes only; chaining from one bakes compression into every later segment:

```
ffmpeg -sseof -0.1 -i seg.mp4 -update 1 -frames:v 1 -q:v 1 final.png
```

This PNG is what you upload through the Magnific bridge (`creations_request_upload` → PUT → `creations_finalize_upload`, spelled out in `magnific.md`), and the returned creation's `identifier` becomes the next segment's `keyframes.start.url`.
